...
export class DersComponent implements OnInit {
  ...
  DersEkleDuzenle() {
    ...
    if (!ders.id) {
      ...
      if (filtre.length > 0) {
        ...
        this.sonuc.mesaj = "Girilen model Adı Kayıtlıdır!";
        ...
        this.servis.DersEkle(ders).subscribe(d => {
          ...
          this.sonuc.mesaj = "model Eklendi";
          this.toast.ToastUygula(this.sonuc);
          this.DersListele();
          this.modal.toggle();
        });
      ...
      this.servis.DersDuzenle(ders).subscribe(d => {
        this.sonuc.islem = true;
        this.sonuc.mesaj = "model Düzenlendi";
        this.toast.ToastUygula(this.sonuc);
        this.DersListele();
        this.modal.toggle();
      });
  ...
  DersSil() {
    this.servis.DersSil(this.secDers.id).subscribe(d => {
      this.sonuc.islem = true;
      this.sonuc.mesaj = "model Silindi";
      this.toast.ToastUygula(this.sonuc);
      this.DersListele();
      this.modal.toggle();
    });
